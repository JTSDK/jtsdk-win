...
CLS
CD /D %based%
SET burl=http://svn.code.sf.net/p/wsjt/wsjt/trunk
SET nopt=wsjt
SET separate=No
SET qt55=No
SET quiet-mode=No
...
CALL %based%\scripts\help\jtsdk-help.cmd wsjthelp
GOTO EOF
) ELSE IF /I [%1]==[-h] (
CALL %based%\scripts\help\jtsdk-help.cmd wsjthelp
GOTO EOF
...
:SVN-CHECKOUT
IF NOT EXIST %srcd%\trunk\.svn\NUL (
CD /D %srcd%
ECHO.
ECHO --------------------------------------------
ECHO  SVN Checkout
ECHO --------------------------------------------
ECHO.
ECHO  Checking Out New Version of ^( WSJT ^) from SVN
ECHO.
...
:GET-SVER
svn info %srcd%\trunk |grep "Rev:" |awk "{print $4}" >s.v & SET /p sver=<s.v & rm s.v
GOTO GET-AVER

:GET-AVER
cat %srcd%\trunk\wsjt.py |grep "Title=" |awk "{print $3}" >a.v & SET /p aver=<a.v & rm a.v
GOTO SETUP-DIRS

:SETUP-DIRS
IF /I [%separate%]==[Yes] (
SET buildd=%srcd%\trunk
SET installd=%based%\wsjt\%aver%\%sver%\install
SET logd=%based%\wsjt\%aver%\%sver%\install\Logbook
...
REM  ***************************************************************************
:MAKE-ISS
rm -f %srcd%\trunk\jtsdk.iss >NUL 2>&1
>%srcd%\trunk\jtsdk.iss (
ECHO ; For Use With JTSDK v2
ECHO #define MyAppName "WSJT"
...
ECHO #define MyAppURL "http://physics.princeton.edu/pulsar/k1jt/doc/wsjt/"
...
ECHO DefaultDirName=C:\WSJT\WSJT-{#MyAppVersion}
ECHO DefaultGroupName=WSJT
...
ECHO.
ECHO [Icons]
ECHO Name: "{group}\{#MyAppName}\Documentation\WSJT {#MyAppVersion} User Guide"; Filename: "{app}\wsjt-main-{#MyAppVersion}.html"; WorkingDir: {app}; IconFileName: "{app}\wsjt.ico"
...
:MAKE-INSTALLER
CD /D %srcd%\trunk
mingw32-make INSTALLDIR=%installd% PACKAGEDIR=%pkgd% LOGBOOKDIR=%logd% SRCD=%srcd%\trunk NAME=wsjt VER=%aver% SVER=r%sver% OS=win32 ISS=jtsdk.iss -f %makefile% package
IF ERRORLEVEL 1 (
GOTO BUILD-ERROR
GOTO EOF
)
...
ECHO  Starting ......: wsjt %aver% r%sver%
ECHO.
CD /D %installd%
CALL wsjt.bat
EXIT /B 0
...
ECHO   Target ......^: %topt%
ECHO   SRC .........^: %srcd%\trunk
ECHO   Location ....^: %srcd%\trunk\doc\%docname%
ECHO   SVN URL .....^: %burl%
ECHO.
...
ECHO --------------------------------------------
ECHO.
ECHO  ^build-wsjt was unable to checkout the
...
ECHO --------------------------------------------
ECHO.
ECHO  ^build-wsjt was unable to update ^[ %nopt% ^]
...
ECHO  available targets by typing^: build-wsjt list-targets
ECHO.
